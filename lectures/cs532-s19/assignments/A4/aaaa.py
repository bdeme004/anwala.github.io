...
def friends_from_twitter(target_name, get_followers=False):
    friends_list = list()
    if(get_followers):
...
def get_friends(target_name):
    if infile is not None:
        ...
        try:
            ...
            print("Followers list collected.", flush=True)
...
def get_friends_of_friends(friends_list, get_followers=False):
    try:
        for friend in friends_list:
            try:
                user = api.get_user(friend[0])
                flist = list((user.followers_count, user.friends_count))

                friends_list.append((user.screen_name, flist[int(get_followers)]))
            except tweepy.RateLimitError:
                print("Rate Limit reached-- waiting...", flush=True)
                time.sleep(15 * 60)
...
friends_list = get_friends_of_friends(get_friends(TARGET_NAME), get_followers=False)
follower_list = get_friends_of_friends(get_friends(TARGET_NAME), get_followers=True)

try:
    with open("tw_friends.txt", 'w') as of:
        for line in friends_list:
            of.write(str(line) + "\n")
    with open("tw_following.txt", 'w') as of:
